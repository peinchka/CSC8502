...
Mesh* Mesh::GenerateTriangle() {
	...
	//m->textureCoords[0] = Vector2(0.4f, 0.0f);
	//m->textureCoords[1] = Vector2(0.9f, 1.0f);
	//m->textureCoords[2] = Vector2(0.0f, 1.0f);
...
Mesh* Mesh::GenerateCube() {
	...
	for (int i = 0; i < 8; ++i) {
		m->colours[i] = Vector4(1.0f/i, 1.0f, 1.0f, 1.0f);
	}
...
Mesh* Mesh::GenerateShape(float x0, float y0, float z0, float x1, float y1, float z1, float x2, float y2, float z2) {
	Mesh* m = new Mesh();
	m->numVertices = 3;
	m->vertices = new Vector3[m->numVertices];

	m->vertices[0] = Vector3(x0, y0, z0);
	m->vertices[1] = Vector3(x1, y1, z1);
	m->vertices[2] = Vector3(x2, y2, z2);
...
//	m->colours[3] = Vector4(0.0f, 1.0f, 1.0f, 0.0f);
...
void Mesh::Draw() {
//	GLfloat* newcolours = (GLfloat*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
//	//newcolours[0] = 0.0f;
////	newcolours[1] = 0.0f;*/
//
//	if (newcolours != (GLfloat*)NULL) {
//		for (int i = 0; i < 12; ++i) {
//		//	if ((i + 1) % 4 != 0) {
//				if (newcolours[i] <= 1.0) {
//					newcolours[i] += 0.1 * GLfloat(rand() % 10000) / 10000.0f;
//				}
//				else
//				{
//					newcolours[i] = 0.0f;
